...
async def handle_menu(callback: types.CallbackQuery, state: FSMContext):
    ...
    if callback.data == "menu_account":
        ...
        if refs < 20:
            await callback.answer(f"🚫 Need 20 refs! Current: {refs}", show_alert=True)
        else:
            await callback.message.answer("Select Method:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                ...
                [InlineKeyboardButton(text="Binance ID", callback_data="meth_BinanceID")]
            ]))
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            await callback.message.answer(f"🏆 **Top 10 Referrers:**\n{text}", parse_mode="Markdown")
...
async def process_method(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("Enter your Address/ID:")
...
async def process_address(message: types.Message, state: FSMContext):
    ...
    await message.answer("✅ **Submitted!** Data sent to admin.")
    await state.clear()
